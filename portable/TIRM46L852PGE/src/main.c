...
int main( void )
{
    ...
    BaseType_t taskStatus = xTaskCreate(
        vBlinkyTask,       // Pointer to the task function
        "Blinky Task",     // Text name for debugging
        128,               // Stack size allocated in words (not bytes)
        NULL,              // Parameter passed into the task (not used here)
        1,                 // Task priority (higher numbers = higher priority)
        NULL               // Task handle storage (not used here)
    );
